        ...
        <div class="row">
            <div class="card col-12">
                ...
                <div class="card-block">
                    <form class="col-12" onsubmit="return confirm('Are You Sure?')" action="{{ route('transport-save',[$type]) }}" method="post" enctype="multipart/form-data">
                        {!! csrf_field() !!}
                        <div class="row">
                            ...
                            <div class="col-12 mt-2 mb-2">
                                ...
                                <option {!! (empty(old('class')))? "selected":"" !!} disabled>Choose Class</option>
                                @foreach($data_class as $key)
                                <option value="{{ $key->id_class }}" {!! ($key->id_class == old('class'))? "selected":"" !!}>{{ $key->name_class." | Rp. ".number_format($key->price_ticket,0).",-" }}</option>
                                @endforeach
                            </select>
                            </div>
                            <div class="col-12 mt-2 mb-2">
